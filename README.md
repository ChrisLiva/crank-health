...
| `agent.md`    | The coding-agent brief: ≤ 20 themed tasks, each with a stable ID, a grade impact, an evidence link and a verify command of its own. A task exists where a finding that counted toward a grade — or an advisory one `related` links to it at the same place — sits in a file somebody wrote: not a manifest, not generated (`gen/`, `*.gen.*`, or a glob the repo's own Biome/fallow config excludes), and for a vulnerable dependency, only where a released version fixes it. Order is deterministic: the work that would move a category's letter first, most letters first, then spec §10's fixed category order (security → types → dead code → complexity → duplication → lint → format). |
| `raw/`        | Each tool's own output, as evidence — source excerpts excepted, see below. Nested by what the run was about: `raw/<project-path>/`, `raw/root/` for the root project, `raw/repo/` for a repo-spanning run, and `raw/base/…` for a `--pr` base scan. A package whose own directory is called `root/` or `repo/` gets a trailing `_`, so nothing shares a directory. A stream a tool wrote nothing to leaves no file and no `tools[].raw` entry: an empty file is evidence of nothing, linked as though it were some.                                                                                                                                                                            |
...
version in a per-release manifest (`npx --yes oxlint@1.80.0`, `uvx --quiet ruff@0.16.5`) — never
...
the same evidence as every run after it. No AI, no sampling, stable sort everywhere.
