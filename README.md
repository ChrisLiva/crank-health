...
| `--out <dir>`               | Output directory (default `<path>/.codebase-health/`)         |
...
One run directory, `.codebase-health/` by default, containing its own `.gitignore` (`*`) so it never
shows up in your working tree:
