 ...
 */
const FALLOW = fileURLToPath(new URL('./captured/fallow-dead-code-3.22.0.json', import.meta.url))
const KNIP = fileURLToPath(new URL('./captured/knip-6.34.0.json', import.meta.url))
...
describe('parseDeadCode', () => {
  it('reads entry points, unused files and unused exports from real output', async () => {
    ...
    expect(report.version).toBe('3.22.0')
