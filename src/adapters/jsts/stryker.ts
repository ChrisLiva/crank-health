...
/** The rule ids this runner reports under. */
 ...
 * it. The rest stay in `raw/stryker-mutation-report.json`.
 */
...
export interface Mutant {
  ...
  /** Stryker's mutator, e.g. `ArithmeticOperator`, `ConditionalExpression`. */
 ...
 * Stryker's `json` reporter writes). Exported so a format shift fails a test
...
/**
 * Mutants → the core's vocabulary. The grade is the score (a ratio over the
 * metrics), so `gradeScope` here is the reader's label: a survived mutant is
 * one of the things the score is made of, and the advisory kinds are not.
 *
 ...
 */
export function toPendingFindings(mutants: readonly Mutant[]): PendingFinding[] {
...
function common(mutant: Mutant) {
  return {
    ...
    // Stryker only ever runs on a repo that owns it, with that repo's config.
