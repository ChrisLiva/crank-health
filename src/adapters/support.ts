 ...
 *
 * @param arrayKey the envelope key holding the results — `results` for bandit
 * and opengrep, `Issues` for gosec. Named by the caller rather than assumed,
 * because a scanner whose key this does not know would have its document handed
 * back untouched: still parseable, still complete, and still quoting every
...
export function sanitizeRawResults(
  ...
  const results = asArray(record?.[arrayKey])
